...
import { Camera, RefreshCw, ChevronRight, Layout, Check, X, Download, Share2, Settings, Plus, Sparkles } from 'lucide-react';
          ...
          {step === 'layout' && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button className="flex items-center gap-2 bg-pink-500 text-white px-6 py-3 rounded-2xl font-semibold shadow-lg shadow-pink-200 hover:bg-pink-600 transition-all active:scale-95">
                  <Plus size={20} />
                  Add Custom Layout
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {TEMPLATES.map((template) => (
                  <motion.div
                    ...
                    className={`group relative bg-white rounded-[32px] overflow-hidden cursor-pointer border-2 transition-all duration-300 ${
                      selectedTemplate?.id === template.id ? 'border-pink-500 shadow-2xl shadow-pink-100' : 'border-transparent hover:shadow-xl'
                    }`}
                  >
              ...
              <div className="fixed bottom-8 left-0 right-0 px-6 pointer-events-none">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                  ...
                  <button
                    ...
                    className={`pointer-events-auto flex items-center gap-2 bg-pink-500 text-white px-10 py-4 rounded-2xl font-bold shadow-xl shadow-pink-200 transition-all active:scale-95 ${
                      !selectedTemplate ? 'opacity-50 cursor-not-allowed grayscale' : 'hover:bg-pink-600'
                    }`}
                  >
                    Next
                ...
                <div className="bg-white p-8 rounded-[40px] shadow-xl border border-black/5">
                  ...
                  <div className="space-y-4 max-w-[280px] mx-auto">
                    {Array.from({ length: selectedTemplate?.slots || 4 }).map((_, i) => (
                      <div 
                        key={i} 
                        className={`aspect-square rounded-2xl overflow-hidden border-2 border-dashed transition-all duration-500 ${
                          photos[i] ? 'border-transparent shadow-md' : 'border-pink-100 bg-pink-50/30'
                        }`}
                      >
                        {photos[i] ? (
